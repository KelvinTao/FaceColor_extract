...
meanAll=c(0,0,0);  #mean value of each channel RGB
meanEach=matrix(0, nrow = length(files), ncol = 3);
...
for(fi in 1:length(files)){
    ...
    meanEach[fi,1]=mean(R);
    meanEach[fi,2]=mean(G);
    meanEach[fi,3]=mean(B);
}

meanAll[1]=mean(meanEach[,1]);
meanAll[2]=mean(meanEach[,2]);
meanAll[3]=mean(meanEach[,3]);
...
for(fi in 1:length(files)){
    ...
    ##normalization
    img[,,1,1]=img[,,1,1]*meanAll[1]/meanEach[fi,1];#mean(R);
    img[,,1,2]=img[,,1,2]*meanAll[2]/meanEach[fi,2];#mean(G));
    img[,,1,3]=img[,,1,3]*meanAll[3]/meanEach[fi,3];#mean(B));
    ##RGB2HSV
    imgHsv=RGBtoHSV(img)
    ##gray for search
