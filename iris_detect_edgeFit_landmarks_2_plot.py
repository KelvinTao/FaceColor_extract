...
def getCondition(light,iris,minus):
	##light: weak, strong; iris color: shallow, dark
	if (light>90 and iris <30) or (light>150 and iris <35) or (light>200 and iris <45): #35,60
...
def getColor(picName,picPath,locPath,lmPath):
	...
	###
	#grayPlot=np.copy(img_gray)
	#grayPlot[topIn,:]=255;grayPlot[downIn,:]=255;grayPlot[:,leftIn]=255;grayPlot[:,rightIn]=255;
	#grayPlot4=np.copy(img_gray)
	#grayPlot4[topIn,leftIn4:rightIn4]=255;grayPlot4[downIn,leftIn4:rightIn4]=255;
	#grayPlot4[topIn:downIn,leftIn4]=255;grayPlot4[topIn:downIn,rightIn4]=255;
	##
	...
	minus=np.mean(img_gray0)-iris##minus not used here
	irisThresh,pad,pupilThresh,condition=getCondition(light,iris,minus)
	#print([np.mean(img_gray0),iris,minus,light])
	...
	##plot line for light and iris initial part for judgement
	img_grayLI=img_gray.copy()
	img_grayLI[topIn,leftIn:rightIn]=255;img_grayLI[downIn,leftIn:rightIn]=255;
	img_grayLI[topIn:downIn,leftIn]=255;img_grayLI[topIn:downIn,rightIn]=255;
	img_grayLI[topIn:downIn,leftIn4]=255;img_grayLI[topIn:downIn,rightIn4]=255;

	if condition=='shallow':
		#######irisUse=np.percentile(img_gray,irisThresh);#90
		...
		for xi in range(h):
			for yi in range(w):
				...
				if img_grayUse[xi,yi]<irisUse and img_grayUse[xi,yi]>pupil:#white
					...
					imgUse[xi,yi,:]=255
					imgShow[x,y,:]=255
		img_gray[topIn,leftIn4:rightIn4]=255;img_gray[downIn,leftIn4:rightIn4]=255;
		img_gray[topIn:downIn,leftIn4]=255;img_gray[topIn:downIn,rightIn4]=255;
		##for color use region show
		[hs,ws,c]=imgShow.shape
		for xs in range(hs):
			for ys in range(ws):
				if not(xs>topIn and xs <downIn and ys>leftIn4 and ys<rightIn4):
					imgShow[xs,ys]=255
		cv2.imwrite(locFile,np.hstack([imgI,img,imgShow]))
		cv2.imwrite(locGFile,np.hstack([img_gray0,closed2,img_grayLI,img_gray]))#,grayPlot4
		...
		for xi in range(h):
			for yi in range(w):
				...
				if dist<=r  and binary0[xi,yi]==255 and closed2[xi,yi]<255:
					...
					imgUse[xi,yi,:]=255
		img_gray[topIn,leftIn:rightIn]=255;img_gray[downIn,leftIn:rightIn]=255;
		img_gray[topIn:downIn,leftIn]=255;img_gray[topIn:downIn,rightIn]=255;
		###save images
		cv2.imwrite(locFile,np.hstack([imgI,img,imgUse]))
		cv2.imwrite(locGFile,np.hstack([img_gray0,closed2,img_grayLI,binary0,closed,canny,img_gray]))
		cv2.imwrite(useFile,imgUse)
...
'''
path='/Users/taoxianming/Documents/face2DExtract/eyebrow_tao/uygur'
picPath=path+'/eye/show/pic_shallow/';#dark
locPath=path+'/eye/show/shallow_loc/';#mkdir(locPath)#drak
###
#picPath=path+'/eye/show/pic_dark/';#dark
#locPath=path+'/eye/show/dark_loc/';#mkdir(locPath)#drak
